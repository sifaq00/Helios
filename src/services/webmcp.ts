...
const TOOL_FAILURE_MESSAGES: Record<WebMcpSpaToolName, string> = {
  openCountryBrief: 'Helios could not open that country brief.',
  openSearch: 'Helios could not open search.',
  get_dashboard_context: 'Helios could not read dashboard context.',
  open_dashboard_panel: 'Helios could not open that dashboard panel.',
  set_map_view: 'Helios could not move the map.',
  set_map_layers: 'Helios could not update map layers.',
  search_dashboard: 'Helios could not search the dashboard.',
  open_search_result: 'Helios could not open that search result.',
};
...
export function buildWebMcpTools(
  ...
  const tools: DashboardWebMcpTool[] = [
    ...
    },
    {
      ...
      title: 'Open Search',
      description:
        'Open the global search command palette so the user can find countries, signals, alerts, and other entities tracked by Helios.',
      inputSchema: {
        type: 'object',
        properties: {},
        additionalProperties: false,
    ...
    },
    {
      ...
      description:
        'Open and scroll to an already-live, currently enabled dashboard panel through the same entitlement-aware control path used by Helios. Disabled panels return panel_disabled. A person can enable them from dashboard search or settings; this tool does not enable panels itself.',
      inputSchema: {
        type: 'object',
        properties: {
          panelId: {
    ...
    },
    {
      ...
      description:
        'Enable or disable explicit visible map layers through Helios’s variant, renderer, and entitlement-aware control path.',
      inputSchema: {
        type: 'object',
        properties: {
          layers: {
    ...
    },
    {
      ...
      description:
        'Search the current Helios country, signal, map, panel, finance, and action indexes without opening the command palette or changing the dashboard.',
      inputSchema: {
        type: 'object',
        properties: {
          query: {
