...
export function buildAgentNotFoundMarkdown(path: string): string {
  ...
  return [
    '# Not found',
    '',
    `\`${safePath}\` is not a page on Helios.`,
    '',
...
export function buildHumanNotFoundHtml(path?: string): string {
  const pathLine = path
    ? `<p><code>${sanitizePathForHtml(path)}</code> is not a page on Helios.</p>`
    : '<p>This path is not a page on Helios.</p>';
  return [
    '<!DOCTYPE html>',
    '<html lang="en">',
    '<head>',
    ...
    '  <title>Page not found — Helios</title>',
    '  <style>',
