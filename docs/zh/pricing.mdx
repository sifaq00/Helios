...
- **Pro** — 涵盖 Free 的全部内容，外加跨实时服务目录并带引用的 WM Analyst 聊天、Scenario Engine、Route Explorer（路线探索器）、个人 AI 摘要（每日 / 每日两次 / 每周，通过 Slack、Discord、Telegram、邮件、webhook 发送）、自定义小部件构建器（10 个自定义仪表盘）、Resilience 地图图层、每天 500 次仪表盘 AI 请求，以及单一密钥下的 MCP 访问（50 次调用/天）。个人许可。
