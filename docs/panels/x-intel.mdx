...
- `@/services/x-intel` — backed by `/api/x-feed`, which proxies to the Railway ais-relay poller. The poller uses the official X API `GET /2/users/:id/tweets` with `since_id` on a 5–15 minute cadence. Account coverage is product-managed rather than user-configurable.
...
The relay polls every 5–15 minutes. The panel refreshes on mount and on the `xIntel` interval in `src/config/variants/base.ts`.
